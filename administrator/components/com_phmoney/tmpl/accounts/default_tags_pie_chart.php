...
if (empty($title)) {
        $title = Text::_('COM_PHMONEY_TAGS_BALANCE');
}
    ...
    <?php foreach ($this->items2 as $i => $item) : ?>
            <?php if (isset($item->portfolio_currency_symbol)) : ?>
                    ...
                    $name = Text::_($item->title)
                            . ' '
                            . PhmoneyHelper::showMoney($item->balance, $item->portfolio_currency_symbol, $item->portfolio_currency_denom, $item->account_type_value)
                            . ' - %' . $item->percentage;
                    ?>
                    <option value="<?php echo PhmoneyHelper::showMoney3($item->balance, $item->portfolio_currency_denom, $item->account_type_value); ?>"><?php echo $name; ?></option>
            <?php endif; ?>
    <?php endforeach; ?>
</select>
