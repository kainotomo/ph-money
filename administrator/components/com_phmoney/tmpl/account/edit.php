...
<form action="<?php echo JRoute::_('index.php?option=com_phmoney&extension=' . $input->getCmd('extension', 'com_phmoney') . '&layout=' . $layout . $tmpl . '&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate" enctype="multipart/form-data">    
    ...
    <div class="card">
        ...
        <div class="nav nav-tabs card-header" id="nav-tab" role="tablist">
            ...
            foreach ($fieldSets as $fieldset_name => $fieldSet) {
                    if ($fieldset_name == 'shares' && $item->account_type_id != 2) {
                            $class = ' hidden';
                    } else {
                            $class = '';
                    }
                    $html = array();
                    $html[] = '<a class="nav-item nav-link' . $class;
                    if ($first) {
                            $html[] = ' active';
                            $first = false;
                    }
