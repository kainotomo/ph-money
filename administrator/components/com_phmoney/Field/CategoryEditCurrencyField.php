...
/**
 * Category edit Field class.
 *
 */
class JFormFieldCategoryEditCurrency extends Field\ListField {
	/**
	 ...
	 * @var    string
	 */
	public $type = 'CategoryEditCurrency';
	...
	protected function getOptions()
	{
		...
		$query = $db->getQuery(true)
			->select("a.id AS value, CONCAT(a.title,' (',cur.symbol,')')  AS text, a.level, a.published, a.lft, a.language")
			->from('#__categories AS a');
                
                // Join over the currency
		$query->join('LEFT', $db->quoteName('#__fields_values') . ' AS fv ON fv.item_id = a.id');
                $query->join('LEFT', $db->quoteName('#__fields') . ' AS f ON f.id = fv.field_id')
                        ->where("f.context LIKE 'com_phmoney.categories' AND f.name LIKE 'currency'");
                $query->select('cur.symbol AS currency_symbol, CONCAT(cur.symbol, a.title) AS mytext')
			->join('LEFT', $db->quoteName('#__phmoney_currencys') . ' AS cur ON cur.id = fv.value');
